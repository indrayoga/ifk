...
class MStokopnamepkm extends MMain {
    function __construct() {
        parent::__construct();
    }

    function getStokopnamepkmData($periodeawal, $periodeakhir, $unit, $puskesmas) {
        ...
        $query = 'select sop.id as id_sop, sop.kd_unit_apt, sop.id_puskesmas, sop.tgl, pkm.nama,u.nama_unit_apt
				from gfk_stokopname_puskesmas sop
	...
	function isNumberExist($number){
		...
		$query=$this->db->get('gfk_stokopname_puskesmas');
	...
	function ambilItemDataStokopname($id){
		$query=$this->db->query("select * from gfk_stokopname_puskesmas where id='$id'");
		return $query->row_array();
	}

	function getAllDetailStokopname($id){ 
		$query=$this->db->query("select * from gfk_stokopname_puskesmas_detail, apt_obat, 
								apt_satuan_kecil
								where 
								gfk_stokopname_puskesmas_detail.kd_obat=apt_obat.kd_obat
								and apt_obat.kd_satuan_kecil=apt_satuan_kecil.kd_satuan_kecil 
								and gfk_stokopname_puskesmas_detail.id='$id' 
