...
class MPemakaian extends MMain {
    ...
    function getKunjunganResepPKMData($bulan,$bulan2,$tahun, $puskesmas) {
    	if(!empty($puskesmas))$pkm=" and a.id_puskesmas='".$puskesmas."'"; else $pkm="";
        $query = 'select *,a.id
				from kunjungan_resep a
				left join gfk_puskesmas b on a.id_puskesmas = b.id
				where bulan >= "'.$bulan.'" and bulan <="'.$bulan2.'" and tahun="'.$tahun.'" '.$pkm.'
				';
	...
	function ambilItemDataKunjunganResep($id){
		$query=$this->db->query("select *,a.id from kunjungan_resep a join gfk_puskesmas b on a.id_puskesmas=b.id where a.id='$id'");
		return $query->row_array();
	}
	...
	function isNumberExistKunjungaResep($number){
		$this->db->where('id',$number);
		$query=$this->db->get('kunjungan_resep');
		$count=$query->num_rows();
		if($count){
			return true;
		}
