...
class Mpersediaanobat extends CI_Model {
	...
	function ambilPersediaan($nama_obat,$stok,$isistok){
		$kd_unit_apt=$this->session->userdata('kd_unit_apt');
		...
		$query=$this->db->query("select apt_stok_unit.kd_obat as kd_obat1,apt_obat.nama_obat,apt_satuan_kecil.satuan_kecil,apt_stok_unit.tgl_expire,apt_stok_unit.harga_pokok,
								apt_stok_unit.jml_stok,(select min_stok from apt_setting_obat where kd_unit_apt='$kd_unit_apt' and kd_obat=kd_obat1) as min_stok, 
								...
								and apt_stok_unit.kd_obat=apt_obat.kd_obat and apt_stok_unit.kd_unit_apt='$kd_unit_apt' and apt_stok_unit.jml_stok $stok1 $isistok1 
								and apt_obat.nama_obat like '%$nama_obat%' order by apt_obat.nama_obat");
		return $query->result_array(); 
	}
