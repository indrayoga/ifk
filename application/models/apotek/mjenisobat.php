...
class Mjenisobat extends CI_Model {
	...
	function ambilDataJenisObat($jenis_obat){
		if(!empty($jenis_obat)) $this->db->like('jenis_obat',$jenis_obat);
			$this->db->order_by('kd_jenis_obat','asc');
			
			$query=$this->db->get('apt_jenis_obat');
			return $query->result_array(); 			
	}
	
	function autoNumber(){
		$this->db->select('max(right(kd_jenis_obat,3)) as a',FALSE);
		$query = $this->db->get('apt_jenis_obat'); 
	...
	function isNumberExist($number){
		$this->db->where('kd_jenis_obat',$number);
		$query=$this->db->get('apt_jenis_obat');
