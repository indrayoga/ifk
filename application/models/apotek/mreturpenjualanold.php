...
class Mreturpenjualan extends CI_Model {
	...
	function ambilDataRetur($no_retur_penjualan,$periodeawal,$periodeakhir){ 
		...
		if(!empty($periodeawal))$this->db->where('date(retur_penjualan.tgl_returpenjualan)>=',convertDate($periodeawal));
		if(!empty($periodeakhir))$this->db->where('date(retur_penjualan.tgl_returpenjualan)<=',convertDate($periodeakhir));
	...
	function ambilStokAwal($kd_unit_apt,$value,$tglexpire){
		$kd_unit_apt=$this->session->userdata('kd_unit_apt');
		$query=$this->db->query("select jml_stok from apt_stok_unit where kd_unit_apt='$kd_unit_apt' and kd_obat='$value' and tgl_expire='$tglexpire'");
	...
	function ambilItemDataTrans($no_retur_penjualan){
		$query=$this->db->query("select retur_penjualan.no_retur_penjualan,retur_penjualan.no_penjualan,retur_penjualan.alasan,retur_penjualan.cust_code,
						...
						retur_penjualan.tutup,date_format(retur_penjualan.tgl_returpenjualan,'%H:%i:%s') as jam_retur from retur_penjualan,apt_customers,apt_penjualan where apt_customers.cust_code=retur_penjualan.cust_code and 
	...
	function ambilItemDataTrans1($no_retur_penjualan){
		$query=$this->db->query("select retur_penjualan.no_retur_penjualan,retur_penjualan.no_penjualan,retur_penjualan.alasan,retur_penjualan.cust_code,
						...
						retur_penjualan.tutup from retur_penjualan,apt_customers,apt_penjualan where apt_customers.cust_code=retur_penjualan.cust_code and 
						retur_penjualan.no_penjualan=apt_penjualan.no_penjualan and retur_penjualan.no_retur_penjualan='$no_retur_penjualan'");
		return $query->row_array();
	}
	...
	function getAllDetailRetur($no_retur_penjualan){ 
		$query=$this->db->query("select retur_penjualan_detail.kd_obat,apt_obat.nama_obat,apt_satuan_kecil.satuan_kecil,date_format(retur_penjualan_detail.tgl_expire,'%d-%m-%Y') as tgl_expire,
	...
	function getdetilpenjualan($no_penjualan){
		$kd_unit_apt=$this->session->userdata('kd_unit_apt');
		$query=$this->db->query("select apt_penjualan_detail.kd_obat,apt_obat.nama_obat,apt_satuan_kecil.satuan_kecil,date_format(apt_penjualan_detail.tgl_expire,'%d-%m-%Y') as tgl_expire,
