...
class Igd extends Unit {
	...
	public function __construct()
	{
		...
		$this->load->model('reg/mrd');
		$this->load->model('migd');

		$this->poli=$this->migd->getKdUnit();
		$this->namapoli=$this->migd->getUnit();
	...
	public function index($kd_pasien="NULL",$nama_pasien="NULL",$periodeawal="NULL",$periodeakhir="NULL",$jns_kelamin="NULL")
	{
		...
		$this->load->view('igd/header',$dataheader);
		$this->load->view('igd/daftar',$data);
		$this->load->view('footer',$datafooter);
	}

	public function pasienigd($kd_pasien="NULL",$nama_pasien="NULL",$periodeawal="NULL",$periodeakhir="NULL",$jns_kelamin="NULL")
	{
		...
		$this->datatables->add_column('pilihan', '<a class="btn btn-info" href="'.base_url().'index.php/igd/periksa/$2">Periksa</a>', 'a.kd_pasien,a.no_pendaftaran');		
		...
		$this->datatables->where('(b.kd_unit_kerja="'.$this->poli.'" or g.kd_unit_tujuan="'.$this->poli.'")');
		//$this->datatables->where('b.kd_unit_kerja','1');
	...
	public function periksa($no_pendaftaran="")
	{
		...
		$this->mrd->setPasien($no_pendaftaran);
		$this->pasien=$this->mrd->pasien;
		//var_dump($this->pasien);
		$data=array(
			'datapelayanan'=>$this->mlayanan->getTarifLayanan(),			
			'datadokter'=>$this->migd->getDokterUnit(),
			'datajenisdiagnosa'=>$this->migd->ambilData('jenis_diagnosa'),
			//'item'=>$this->mpoli->getItemDaftarPasien($no_pendaftaran,$kd_pasien),
			'itemslayanan'=>$this->mrd->pelayananpasien(),
			'itemsdiagnosa'=>$this->mrd->diagnosapasien(),
			'no_pendaftaran'=>$no_pendaftaran
		);
		$this->load->view('igd/header',$dataheader);
		$this->load->view('igd/periksapasien',$data);
		$this->load->view('footer',$datafooter);
	}
	...
	public function simpandiagnosa()
	{
		...
		$this->mrd->setPasien($no_pendaftaran);
		$this->pasien=$this->mrd->pasien;
		if($this->mrd->isPasienDiagnosaExist($urut)){
		...
		if($this->mrd->isPasienDiagnosaLama($kd_sub_icd)){
			$status="Lama";
		}else{
			$status="Baru";
		}
		$urut=$this->mrd->getNextNoUrutDiagnosa();
		//$urut=$urut+1;
		$data_diagnosa=array(
		...
		$this->migd->insert('periksa_diagnosa',$data_diagnosa);
		$msg['urut']=$urut;
		//$urut=1;
	...
	public function simpanpelayanan()
	{
		...
		$itemlayanan1=$this->migd->ambilItemData('biaya_pelayanan','no_pendaftaran = "'.$no_pendaftaran.'" and kd_jenis_tarif="'.$kd[0].'" and kd_pelayanan="'.$kd[1].'" and kd_kelas="'.$kd[2].'" and tgl_berlaku="'.$kd[3].'"');
		...
		$urut=$this->mrd->getNextNoUrutPelayanan();
		$itemlayanan=$this->migd->ambilItemData('tarif','kd_jenis_tarif="'.$kode[0].'" and kd_pelayanan="'.$kode[1].'" and kd_kelas="'.$kode[2].'" and tgl_berlaku="'.$kode[3].'"');
		...
		$this->migd->insert('biaya_pelayanan',$data_pelayanan);
		$this->mrd->insertbiayapelayanancomponent(0,$no_pendaftaran,$urut,$itemlayanan['kd_jenis_tarif'],$itemlayanan['kd_pelayanan'],$itemlayanan['kd_kelas'],$itemlayanan['tgl_berlaku']);
	...
	public function konsul($no_pendaftaran="")
	{
		...
		$data=array(
			'datadokter'=>$this->migd->getDokterUnit(),
			'unittujuan'=>$this->migd->ambilData("unit_kerja")
		);
		$this->load->view('igd/header',$dataheader);
	...
	public function periksakonsul()
	{
		...
		$itemkonsul=$this->migd->ambilItemData('pasien_rujukan','no_pendaftaran="'.$no_pendaftaran.'"');
		if(!empty($itemkonsul)){
			$jumlaherror++;
			$msg['id'][]="no_pendaftaran";
	...
	public function simpankonsul()
	{
		...
		$this->migd->insert('pasien_rujukan',$data);
		echo json_encode($msg);
	}
	...
	public function hapusdiagnosa()
	{
		...
		$this->migd->delete('periksa_diagnosa','no_pendaftaran = "'.$no_pendaftaran.'" and urut="'.$urut.'"');
	...
	public function hapuspelayanan()
	{
		...
		$this->migd->delete('biaya_pelayanan','no_pendaftaran = "'.$no_pendaftaran.'" and urut="'.$urut.'"');
		$this->migd->delete('biaya_pelayanan_component','no_pendaftaran = "'.$no_pendaftaran.'" and urut="'.$urut.'"');
	...
	public function datatableslistpelayanan()
	{
		...
		$this->datatables->where('a.kd_kelas','07');
