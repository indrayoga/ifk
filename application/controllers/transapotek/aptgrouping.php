	...
	public function simpangrouping(){
		...
		if($submit=="simpan"){
			if($this->mgrouping->isNumberExist($no_grouping)){ //edit
				$this->db->trans_start();
				$this->db->query("SELECT * FROM apt_grouping_pengajuan where no_grouping='".$no_grouping."' FOR UPDATE");
				...
				$this->mgrouping->update('apt_pengajuan',$group,'is_grouping="0" AND status_approve=1');
				$this->db->trans_complete();
