...
class Returpenjualanobat extends CI_Controller {
	...
	public function tambahretur(){	
		...
		$data=array('no_retur_penjualan'=>'',
					...
					'items'=>$this->mreturpenjualan->ambilDataRetur('','','')
					);
	...
	public function simpanretur(){
		...
		if($submit=="tutuptrans"){
			...
			if(!empty($kd_obat)){
				foreach ($kd_obat as $key => $value){
					...
					$jml_stok_awal=$this->mreturpenjualan->ambilStokAwal($kd_unit_apt,$value,$tglexpire);
					$sisastok=$jml_stok_awal+$qty[$key];
					$datastok=array('jml_stok'=>$sisastok);
					$this->mreturpenjualan->update('apt_stok_unit',$datastok,'kd_unit_apt="'.$kd_unit_apt.'" and kd_obat="'.$value.'" and tgl_expire="'.$tglexpire.'"');					
				}
			}
		...
		if($this->mreturpenjualan->isNumberExist($no_retur_penjualan)){ //edit
			$tgl_returpenjualan1=convertDate($tgl_returpenjualan)." ".$jam_retur1;
			...
			$msg['no_retur_penjualan']=$no_retur_penjualan;
			$tgl_returpenjualan1=convertDate($tgl_returpenjualan)." ".$jam_retur;
		...
		if($submit=="hapus"){
			...
			if(!empty($kd_obat)){
				foreach ($kd_obat as $key => $value) {
					...
					if($qty[$key]>$qty1[$key]){
						$msg['status']=0;
						$msg['pesanlain'].="Jumlah Qty ".$value." yang diretur tidak boleh melebihi jumlah penjualan<br/>";					
					}
				}
			}
			if($jumlaherror>0){
