...
class Penerimaan extends CI_Controller {
	...
	public function simpanpenerimaan(){
		...
		if($this->mpenerimaan->isNumberExist($no_penerimaan)){ //edit
		    $datapenerimaanedit=array('tgl_penerimaan'=>convertDate($tgl_penerimaan),'kd_lokasi'=>$kd_lokasi,
										'keterangan'=>$keterangan,'status_penerimaan'=>$status_penerimaan,
										'kd_user'=>0,'no_spk'=>$no_spk,'tgl_spk'=>convertDate($tgl_spk));
	...
	public function ambildaftarbarangbynama1(){
		$q=$this->input->get('query');
		$items=$this->mpenerimaan->daftarbarang($q);
		echo json_encode($items);
	}
