...
class Monitoring extends CI_Controller {
	...
	public function __construct(){
		...
		$this->load->model('logistik/mmonitoring');
	}
	...
	public function index()	{
		...
		$data=array('nama_barang'=>$nama_barang,
					'kd_barang'=>$kd_barang,
					'items'=>$this->mmonitoring->getData($nama_barang,$kd_barang,$kd_lokasi));
		
		$this->load->view('headerlogistik',$dataheader);
		$this->load->view('logistik/transaksi/monitoring/monitoring',$data);
		$this->load->view('footer',$datafooter);
	}
	
	public function ambilbarangbynama(){
		$q=$this->input->get('query');
		$items=$this->mmonitoring->ambilBarang($q);
