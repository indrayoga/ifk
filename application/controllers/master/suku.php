...
class Suku extends CI_Controller {
	...
	public function index()
	{
		...
		$data=array(
			'items'=>$this->mmaster->ambilData('suku','suku like "%'.$suku.'%"')
			
		);
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/suku',$data);
	...
	public function tambah()
	{
		...
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/tambahsuku',$data);
		$this->load->view('footer',$datafooter);
	}
	...
	public function periksa()
	{
		...
		if($mode!="edit"){
			if($this->mmaster->isExist('suku','kd_suku',$kd_suku)){
				$jumlaherror++;
				$msg['id'][]="kd_suku";
	...
	public function update(){
		...
		$this->mmaster->update('suku',$data,'kd_suku="'.$kd_suku.'"');		
		$msg['pesan']="Data Berhasil Di Simpan";
		$msg['status']=1;
	...
	public function edit($id=""){
		...
		$data=array(
			'item'=>$this->mmaster->ambilItemData('suku','kd_suku="'.$id.'"'),
		);
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/editsuku',$data);
		$this->load->view('footer',$datafooter);

	}
	public function hapus($id=""){
		if(!empty($id)){
			$this->mmaster->delete('suku','kd_suku="'.$id.'"');
			redirect('/master/suku');
		}
	}
}
