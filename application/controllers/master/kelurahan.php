...
class Kelurahan extends Rumahsakit {
	...
	public function index()
	{
		...
		$data=array(
			'items'=>$this->mmaster->ambilDataRelasi('kelurahan a','kecamatan b','a.kd_kecamatan=b.kd_kecamatan','kelurahan like "%'.$kelurahan.'%"')
	...
	public function tambah()
	{
		...
		$data=array(
			'datakecamatan'=>$this->mmaster->ambilData('kecamatan')
		);

		$this->load->view('master/header',$dataheader);
		$this->load->view('master/tambahkelurahan',$data);
		$this->load->view('footer',$datafooter);
	}
	...
	public function periksa()
	{
		...
		if($mode!="edit"){
			if($this->mmaster->isExist('kelurahan','kd_kelurahan',$kd_kelurahan)){
				$jumlaherror++;
				$msg['id'][]="kd_kelurahan";
		...
		if(empty($kelurahan)){
			$jumlaherror++;
			$msg['id'][]="kelurahan";
			$msg['pesan'][]="Kolom kelurahan Harus di Isi";
		}
	...
	public function simpan(){
		...
		$this->mmaster->insert('kelurahan',$data);		
	...
	public function update(){
		...
		$this->mmaster->update('kelurahan',$data,'kd_kelurahan="'.$kd_kelurahan.'"');		
		$msg['pesan']="Data Berhasil Di Simpan";
		$msg['status']=1;
	...
	public function edit($id=""){
		...
		$data=array(
			'item'=>$this->mmaster->ambilItemData('kelurahan','kd_kelurahan="'.$id.'"'),
			'datakecamatan'=>$this->mmaster->ambilData('kecamatan')
		);
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/editkelurahan',$data);
		$this->load->view('footer',$datafooter);

	}
	public function hapus($id=""){
		if(!$this->muser->isAkses("933")){
			$this->restricted();
			return false;
		}
		if(!empty($id)){
			$this->mmaster->delete('kelurahan','kd_kelurahan="'.$id.'"');
