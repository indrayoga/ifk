...
class Subdiagnosaicd extends Rumahsakit {
	...
	public function index()
	{
		...
		$data=array(
			'items'=>$this->mmaster->ambilSubDiagnosaICD($diagnosa)
			
		);
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/subdiagnosaicd',$data);
	...
	public function tambah()
	{
		...
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/tambahsubdiagnosaicd',$data);
		$this->load->view('footer',$datafooter);
	}
	...
	public function periksa()
	{
		...
		$sub_diagnosa_icd=$this->input->post('sub_diagnosa_icd');
		$jumlaherror=0;
		$msg['status']=1;
		$msg['clearform']=0;
		$msg['pesanatas']="";
		...
		if(empty($kd_sub_icd)){
			$jumlaherror++;
			$msg['id'][]="kd_sub_icd";
			$msg['pesan'][]="Kd. Sub ICD Harus di Isi";
		}

		if($mode!="edit"){
			if($this->mmaster->isExist('sub_diagnosa_icd','kd_sub_icd',$kd_sub_icd)){
				$jumlaherror++;
				$msg['id'][]="kd_sub_icd";
				$msg['pesan'][]="Kd. sub ICD sudah ada";
			}			
		}

		if(empty($sub_diagnosa_icd)){
			$jumlaherror++;
			$msg['id'][]="sub_diagnosa_icd";
			$msg['pesan'][]="Kolom sub diagnosa icdHarus di Isi";
		}
	...
	public function simpan(){
		...
		$data=array(
			...
			'sub_diagnosa_icd'=>$sub_diagnosa_icd
		);
		$this->mmaster->insert('sub_diagnosa_icd',$data);
	...
	public function update(){
		...
		$this->mmaster->update('sub_diagnosa_icd',$data,'kd_sub_icd="'.$kd_sub_icd.'"');
	...
	public function edit($id=""){
		...
		$data=array(
			'item'=>$this->mmaster->ambilItemData('sub_diagnosa_icd','kd_sub_icd="'.urldecode($id).'"')
		);
		//debugvar($data);
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/editsubdiagnosaicd',$data);
		$this->load->view('footer',$datafooter);
	...
	public function hapus($id=""){
		...
		if(!empty($id)){
			$this->mmaster->delete('sub_diagnosa_icd','kd_sub_icd="'.$id.'"');
			redirect('/master/subdiagnosaicd');
		}
	}
}
