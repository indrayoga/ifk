...
class Listpelayanan extends Rumahsakit {
	...
	{
		if(!$this->muser->isAkses("938")){
			$this->restricted();
			return false;
		}
		$listpelayanan=$this->input->post('listpelayanan');
		...
		$data=array(
			'items'=>$this->mmaster->ambilDataRelasi('list_pelayanan a','jenis_pelayanan b','a.kd_jenis_pelayanan=b.kd_jenis_pelayanan','nama_pelayanan like "%'.$listpelayanan.'%"')
			
		);
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/listpelayanan',$data);
		$this->load->view('footer',$datafooter);
	}
	
	public function tambah()
	{
		if(!$this->muser->isAkses("939")){
			$this->restricted();
			return false;
		}
		...
		$data=array(
			'datajenispelayanan'=>$this->mmaster->ambilData('jenis_pelayanan')
		);

		$this->load->view('master/header',$dataheader);
		$this->load->view('master/tambahlistpelayanan',$data);
		$this->load->view('footer',$datafooter);
	}
	...
	public function periksa()
	{
		...
		if(empty($kd_pelayanan)){
			$jumlaherror++;
			$msg['id'][]="kd_pelayanan";
			$msg['pesan'][]="Kode Pelayanan Harus di Isi";
		}

		if(empty($kd_jenis_pelayanan)){
			$jumlaherror++;
			$msg['id'][]="kd_jenis_pelayanan";
			$msg['pesan'][]="Jenis Pelayanan Harus di Isi";
		}

		if($mode!="edit"){
			if($this->mmaster->isExist('list_pelayanan','kd_pelayanan',$kd_pelayanan)){
				$jumlaherror++;
				$msg['id'][]="kd_pelayanan";
		...
		if(empty($nama_pelayanan)){
			$jumlaherror++;
			$msg['id'][]="nama_pelayanan";
			$msg['pesan'][]="Kolom Pama Pelayanan Harus di Isi";
		}
	...
	public function simpan(){
		...
		$this->mmaster->insert('list_pelayanan',$data);		
	...
	public function update(){
		...
		$this->mmaster->update('list_pelayanan',$data,'kd_pelayanan="'.$kd_pelayanan.'"');		
		$msg['pesan']="Data Berhasil Di Simpan";
		$msg['status']=1;
	...
	public function edit($id=""){
		...
		$data=array(
			'item'=>$this->mmaster->ambilItemData('list_pelayanan','kd_pelayanan="'.$id.'"'),
			'datajenispelayanan'=>$this->mmaster->ambilData('jenis_pelayanan')
		);
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/editlistpelayanan',$data);
		$this->load->view('footer',$datafooter);

	}
	public function hapus($id=""){
		if(!$this->muser->isAkses("941")){
			$this->restricted();
			return false;
		}
		if(!empty($id)){
			$this->mmaster->delete('list_pelayanan','kd_pelayanan="'.$id.'"');
			redirect('/master/listpelayanan');
		}
	}
}
