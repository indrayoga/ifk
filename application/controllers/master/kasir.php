...
class Kasir extends Rumahsakit {
	...
	public function index()
	{
		...
		$data=array(
			'items'=>$this->mmaster->ambilData("kasir",'kasir like "%'.$kasir.'%" '),
			
		);
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/kasir',$data);
		$this->load->view('footer',$datafooter);
	}
	
	public function tambah()
	{
		if(!$this->muser->isAkses("964")){
			$this->restricted();
			return false;
		}
		...
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/tambahkasir',$data);
		$this->load->view('footer',$datafooter);
	}
	...
	public function periksa()
	{
		...
		if($mode!="edit"){
			if($this->mmaster->isExist('kasir','kd_kasir',$kd_kasir)){
				$jumlaherror++;
				$msg['id'][]="kd_kasir";
		...
		if(empty($kasir)){
			$jumlaherror++;
			$msg['id'][]="kasir";
			$msg['pesan'][]="Kolom Kasir Harus di Isi";
		}
	...
	public function simpan(){
		...
		$this->mmaster->insert('kasir',$data);		
	...
	public function update(){
		...
		$this->mmaster->update('kasir',$data,'kd_kasir="'.$kd_kasir.'"');		
	...
	public function edit($id=""){
		...
		$data=array(
			'item'=>$this->mmaster->ambilItemData('kasir','kd_kasir="'.$id.'"')
		);
		$this->load->view('master/header',$dataheader);
		$this->load->view('master/editkasir',$data);
		$this->load->view('footer',$datafooter);

	}
	public function hapus($id=""){
		if(!$this->muser->isAkses("966")){
			$this->restricted();
			return false;
		}
		if(!empty($id)){
			$this->mmaster->delete('kasir','kd_kasir="'.$id.'"');
