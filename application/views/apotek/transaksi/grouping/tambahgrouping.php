                                        ...
                                        <header class="top" style="">
                                            ...
                                            <div class="toolbar" style="height:auto;">
                                                <ul class="nav nav-tabs">
													...
													<li><button class="btn" id="buatpesanan" type="submit" name="submit" value="buatpesanan" <?php if(empty($no_grouping)){ ?>disabled<?php } else{ if($this->mgrouping->isPosted($no_grouping))echo "disabled"; else {echo "enabled";} } ?>> <i class="icon-plus"></i> Buat Pesanan</button></li>
													<li>
													...
													<!--div class="span8"-->
														<div class="control-group">
															...
															<div class="controls with-tooltip">
																...
																<input type="hidden" id="kd" name="kd" value="" class="input-mini" data-original-title="kd" data-placement="bottom"/>
															</div>
														</div>
													<!--/div-->													
												</div>
																	...
																	foreach ($itemsdetiltransaksi as $itemdetil){																			
																		//if($itemdetil['jml_penerimaan']>0) { ?>
																			<tr><!--td style="text-align:center;"><input type="checkbox" class="barisinput cleared" /></td-->
																				...
																				<td><input <?php if($this->mgrouping->isPosted($no_grouping))echo "readonly"; ?> type="text" name="satuan_kecil[]" value="<?php echo $itemdetil['satuan_kecil'] ?>" style="width:100px;" class="input-medium barissatuan cleared" readonly ></td>
																				...
																				<td style="text-align:right;"><input type="text" style="text-align:right;" name="total[]" value="<?php echo number_format($itemdetil['total'],2,'.','') ?>" style="width:130px;" class="input-medium barisjumlah cleared" readonly ></td>																			
																				<!--td style="text-align:right;"><input type="text" style="background:green; color:white;text-align:right;" name="total[]" value="<-?php echo number_format($itemdetil['total'],2,'.','') ?>" style="width:130px;" class="input-medium barisjumlah cleared" readonly ></td-->
																			</tr>
																		<?php //} 
																	}
																}
...
<script type="text/javascript">
	...
	$('#buatpesanan').click(function(){
		var tanya = confirm("Anda yakin ingin membuat pesanan ?")
		var nomor=$('#no_grouping').val();
		if (tanya){
			$('#kd').val(1);
			$.ajax({
				//url: '<?php echo base_url() ?>index.php/masterapotek/persediaan/hapussubmit/',
				//url: '<?php echo base_url() ?>index.php/transapotek/aptgrouping/buatpemesanan/',
				url: '<?php echo base_url() ?>index.php/transapotek/aptgrouping/simpangrouping/',
				async:false,
				type:'post',
				data:{query:$('#no_grouping').val()},
				success:function(data){	
					//window.location.href='<?php echo base_url(); ?>index.php/masterapotek/persediaan/persediaanobat';
					alert("Pemesanan berhasil disimpan")
					//window.location.href='<?php echo base_url(); ?>index.php/transapotek/aptgrouping/ubahgrouping/'+nomor;
					window.location.href='<?php echo base_url(); ?>index.php/transapotek/aptpemesanan/';
				},
				dataType:'json'                         
			});
	...
	/*$('#batal').click(function(){
		alert('masuk')
		var answer = confirm("Anda yakin ingin membuat pesanan ?");
		var nomor=$('#no_grouping').val();
		alert('1');
		if (answer){
			alert('2');
