    ...
    $(document).ready(function() {
            ...
            $('#form').ajaxForm({
                beforeSubmit: function(a,f,o) {
                    ...
                    z=true;
                    $.ajax({
                    url: "<?php echo base_url(); ?>index.php/master/kasirunit/periksa",
                    type:"POST",
                    async: false,
                    data: $.param(a),
                                ...
                                <div class="span12">
                                    <div class="box">
                                        <header>
                                            ...
                                            <div class="toolbar" style="height:auto;">
                                                <ul class="nav nav-tabs">
                                                    <li><a href="<?php echo base_url() ?>index.php/master/kasirunit"> <i class="icon-list"></i> Daftar</a></li>
                                                    <!--<li class="dropdown">
                                        ...
                                        <div id="div-1" class="accordion-body collapse in body">
                                            <form class="form-horizontal" id="form" method="post" action="<?php echo base_url()?>index.php/master/kasirunit/update">
                                                <input type="hidden" name="mode" value="edit">
                                                <div class="row-fluid">
                                                    <div class="span12">
                                                        <div class="span5">
                                                            <div class="control-group">
                                                                <label for="kd_kasir" class="control-label">Kasir</label>
                                                                <div class="controls with-tooltip">
                                                                    <select name="kd_kasir" class="input-xlarge">
                                                                        <?php
                                                                        foreach ($datakasir as $kasir) {
                                                                            # code...
                                                                            $select="";
																			if(isset($item['kd_kasir'])){    
																				if($item['kd_kasir']==$kasir['kd_kasir'])$select="selected=selected";else $select="";
																			}
                                                                        ?>
                                                                        <option value="<?php echo $kasir['kd_kasir'] ?>" <?php echo $select; ?>><?php echo $kasir['kasir'] ?></option>
                                                                        <?php
                                                                        }
                                                                        ?>
                                                                    </select>
                                                                     <span class="help-inline"></span>
																	 <input type="hidden" id="kd_kasir1" name="kd_kasir1" class="input-small input-tooltip"
                                                                        value="<?php if(isset($item['kd_kasir']))echo $item['kd_kasir'] ?>"   data-original-title="masukkan kode " data-placement="bottom"/>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="row-fluid">
                                                    <div class="span12">
                                                        <div class="span5">
                                                            <div class="control-group">
                                                                <label for="kd_unit_kerja" class="control-label">Unit Kerja</label>
                                                                <div class="controls with-tooltip">
                                                                    <select name="kd_unit_kerja" class="input-xlarge">
                                                                        <?php
                                                                        foreach ($dataunit as $unit) {
                                                                            # code...
																			$select="";
																			if(isset($item['kd_unit_kerja'])){    
																				if($item['kd_unit_kerja']==$unit['kd_unit_kerja'])$select="selected=selected";else $select="";
																			}
                                                                        ?>
                                                                        <option value="<?php echo $unit['kd_unit_kerja'] ?>" <?php echo $select; ?>><?php echo $unit['nama_unit_kerja'] ?></option>
                                                                        <?php
                                                                        }
                                                                        ?>
                                                                    </select>
                                                                     <span class="help-inline"></span>																	 																 
																	 <input type="hidden" id="kd_unit_kerja1" name="kd_unit_kerja1" class="input-small input-tooltip"
                                                                        value="<?php if(isset($item['kd_unit_kerja']))echo $item['kd_unit_kerja'] ?>"   data-original-title="masukkan kode " data-placement="bottom"/>
                                                                </div>
                                                            </div>
                                                        </div>														
                                                    </div>
