		...
		$('#form').ajaxForm({
            ...
            //data = objToString(data);
                if(parseInt(data.status)==1) //jika berhasil
                {
					...
					var totalakhir=0;
                    $('#listmutasi').empty();
                    $.each(data.items,function(i,l){
                        //alert(l);
						//console.log(l.nama_obat);	
						/*var harga;
						if(l.harga_beli==null){harga='0';}
						else {harga=l.harga_beli;}
						
						if(l.total_awal==null){totalawal1='0';}
						else {totalawal1=l.total_awal;}
						
						if(l.total_masuk==null){totalmasuk1='0';}
						else {totalmasuk1=l.total_masuk;}
						
						if(l.total_keluar==null){totalkeluar1='0';}
						else {totalkeluar1=l.total_keluar;}
						
						if(l.total_akhir==null){totalakhir1='0';}
						else {totalakhir1=l.total_akhir;}*/
						
						/*totalawal=totalawal+parseInt(totalawal1);
						totalmasuk=totalmasuk+parseInt(totalmasuk1);
						totalkeluar=totalkeluar+parseInt(totalkeluar1);
						totalakhir=totalakhir+parseInt(totalakhir1);*/
						
                        $('#listmutasi').append('<tr style="font-size:80% !important;"><td style="text-align:center;width:5px !important;">'+no+'</td>'+
													'<td style="width:25px !important;">'+l.nama_obat+'</td>'+
													'<td style="text-align:right;width:15px !important;">'+parseFloat(l.saldo_awal).toFixed(2)+'</td>'+
													//'<td style="text-align:right;width:25px !important;">'+addCommas(harga)+'</td>'+
													//'<td style="text-align:right;width:25px !important;">'+addCommas(totalawal1)+'</td>'+
													'<td style="text-align:right;width:15px !important;">'+parseFloat(l.in_pbf).toFixed(2)+'</td>'+ 
													'<td style="text-align:right;width:15px !important;">'+parseFloat(l.in_unit).toFixed(2)+'</td>'+
													//'<td style="text-align:right;width:15px !important;">'+parseFloat(l.retur_pbf).toFixed(2)+'</td>'+
													'<td style="text-align:right;width:15px !important;">'+parseFloat(l.retur_jual).toFixed(2)+'</td>'+
													'<td style="text-align:right;width:15px !important;">'+parseFloat(l.jum_masuk).toFixed(2)+'</td>'+
													//'<td style="text-align:right;width:25px !important;">'+addCommas(totalmasuk1)+'</td>'+
													'<td style="text-align:right;width:15px !important;">'+parseFloat(l.out_jual).toFixed(2)+'</td>'+
													'<td style="text-align:right;width:15px !important;">'+parseFloat(l.out_unit).toFixed(2)+'</td>'+
													'<td style="text-align:right;width:15px !important;">'+parseFloat(l.retur_pbf).toFixed(2)+'</td>'+
													//'<td style="text-align:right;width:15px !important;">'+parseFloat(l.retur_jual).toFixed(2)+'</td>'+
													'<td style="text-align:right;width:15px !important;">'+parseFloat(l.jum_keluar).toFixed(2)+'</td>'+
													//'<td style="text-align:right;width:25px !important;">'+addCommas(totalkeluar1)+'</td>'+
													'<td style="text-align:right;width:25px !important;">'+parseFloat(l.stok_opname).toFixed(2)+'</td>'+
													'<td style="text-align:right;width:25px !important;">'+parseFloat(l.saldo_akhir).toFixed(2)+'</td></tr>');
													//'<td style="text-align:right;width:25px !important;">'+addCommas(totalakhir1)+'</td></tr>');													
                        no++;
                    });
                    //$('#totalawal').html(addCommas(totalawal));
					//$('#totalmasuk').html(addCommas(totalmasuk));
					//$('#totalkeluar').html(addCommas(totalkeluar));
					//$('#totalakhir').html(addCommas(totalakhir));
                    //$('#detilcetak').attr('href','<?php echo base_url() ?>third-party/fpdf/mutasiobat.php?kd_unit_apt='+data.kd_unit_apt+'&bulan='+data.bulan+'&tahun='+data.tahun+'');
					$('#detilcetak').attr('href','<?php echo base_url() ?>index.php/transapotek/laporanapt/excelmutasiobat/'+data.kd_unit_apt+'/'+data.bulan+'/'+data.tahun);
                    $('#detilmutasi').modal('show');                    
                }
                                ...
                                <div class="span12">
                                    <div class="box">
                                        ...
                                        <div id="div-1" class="accordion-body collapse in body">
                                            <form id="form" class="form-horizontal" method="POST" action="<?php echo base_url() ?>index.php/transapotek/laporanapt/carimutasiobat">                                                
												<div class="row-fluid">
                                                    <div class="span12">
                                                        <div class="span7">
                                        ...
                                        <div aria-hidden="true" aria-labelledby="approveModalLabel" role="dialog" tabindex="-1" class="modal hide fade" id="detilmutasi" style="display: none;width:92%;margin-left:30;left:10px;top:25px;">
                                            <div class="modal-header">
                                               ...
                                               <!--
                                                ...
                                                -->                 
                                                <h3 id="helpModalLabel"><i class="icon-external-link"></i> LPLPO</h3>
                                            </div>
